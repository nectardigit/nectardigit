                ...
                <div class="card-header">
                    <div class="row">
                        <div class="btn-group col-lg-1">
                            <a href="{{ route('notice.index') }}" class="btn btn-primary btn-flat btn-sm">
                                <i class="fas fa-sync-alt fa-sm"></i> Refresh
                            </a>
                        </div>
                        <div class="col-lg-7">
                            <form action="{{route('notice.index')}}" class="" method="get">
                                <div class="row">
                        ...
                        <div class="col-lg-4">
                            <div class="card-tools float-right">
                                @can('notice-create')
                                    <a href="{{ route('notice.create') }}" class="btn btn-success btn-sm btn-flat mr-2">
                            ...
                            @foreach ($data as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            @can('notice-edit')
                                                <a href="{{ route('notice.edit', $value->id) }}" title="Edit Notice"
                                                    class="btn btn-success btn-sm btn-flat">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                            @endcan
                                            @can('notice-delete')
                                                {{ Form::open(['method' => 'DELETE', 'route' => ['notice.destroy', $value->id], 'style' => 'display:inline', 'onsubmit' => 'return confirm("Are you sure you want to delete this Notice?")']) }}
                                                {{ Form::button('<i class="fas fa-trash-alt"></i>', ['class' => 'btn btn-danger btn-sm btn-flat', 'type' => 'submit', 'title' => 'Delete Notice ']) }}
                                                {{ Form::close() }}
                                            @endcan
                                        </div>
                                    </td>
