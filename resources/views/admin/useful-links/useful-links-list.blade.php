            ...
            $("#serialize").click(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        toastr.success('Useful links Order updated Successfuly', "Success !");
                        $('#serialize').prop("disabled", false);
                        $('#serialize').html(`<i class="fa fa-save"></i> Update Useful Links`);
                    }
                });
            });
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Manage Useful Links</h3>
                    <div class="card-tools">
                        <a href="{{ route('usefullinks.index') }}" type="button" class="btn btn-tool">
                            <i class="fa fa-list"></i></a>
                    </div>
                </div>
                <div class="card-header">
                    <div class="btn-group">
                        <a href="{{ route('usefullinks.index') }}" class="btn btn-primary btn-flat btn-sm">
                            <i class="fas fa-sync-alt fa-sm"></i> Refresh
                        </a>
                    </div>
                    <div class="card-tools">
                        @can('menu-create')
                            <a href="{{ route('usefullinks.create') }}" class="btn btn-success btn-sm btn-flat mr-2">
                                <i class="fa fa-plus"></i> Add  useful link</a>
                        @endcan
                    </div>
                </div>
                    ...
                    @if ($data->count() > 0)
                        <ol class="sortable">
                                    ...
                                    <li id="menuItem_{{ $item->id }}">
                                        <div class="menu-handle d-flex justify-content-between">
                                            ...
                                            <div class="menu-options btn-group">
                                                ...
                                                {{-- <a href="#" class="btn btn-xs btn-{{ $item->publish_status == '1' ? 'info' : 'danger' }}" title="{{ $item->publish_status ? 'Unpublish This Menu' : 'Publish This Menu' }}">
                                                    <i class="fa {{ $item->publish_status == '1' ? 'fa-eye' : 'fa-eye-slash' }}"></i>
                                                </a> --}}
                                                {{-- <a href="{{ validate_url($item->slug) ? $item->slug : Config::get('APP_URL') . '/' . $item->slug ?? $item->slug }}"
                                                    target="__banner" class="btn btn-xs btn-primary">
                                                    <i class="fa fa-link"></i>
                                                </a> --}}

                                                @can('menu-edit')
                                                    <a href="{{ route('usefullinks.edit', $item->id) }}"
                                                        class="btn btn-xs btn-success">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                @endcan
                                                @can('menu-delete')
                                                    {{ Form::open(['method' => 'DELETE', 'route' => ['usefullinks.destroy', $item->id], 'class' => 'display:hidden', 'id' => $item->id, 'onsubmit' => 'return confirm("Are you sure you want to delete this User?")']) }}
                                                    {{ Form::close() }}
                            ...
                            <div class="form-group mt-4">
                                ...
                                @endcanany
                                {{-- <a href="{{ route('menu.resetoreder') }}" type="button"
                                    class="btn btn-danger btn-sm btn-flat">
                                    <i class="fas fa-sync-alt"></i> Reset Order
                                </a> --}}
                            </div>
                        </ol>
                    @else
